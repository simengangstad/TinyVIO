...
__attribute__((naked, section(".after_vectors.reset"))) void ResetISR(void) {
    ...
    __asm volatile("MSR MSP, %0" : : "r"(&__stack_top) :);

    // FlexRAM configuration for DTCM 480 KB and ITCM 32KB by modifying
    // IOMUXC_GPR 17 and 18. In the registers every two bit is one
    // bank.
    __asm volatile("DSB\n"
                   "ISB\n"
                   ...
                   "LDR R0,=0x400E4048\n" // Load IOMUXC_GPR18 register
                                          // address to R0
                   "LDR R1,=0x0000EAAA\n" // Move FlexRAM configuration for
                                          // GPR18 value to R1. 7 banks for DTCM
                                          // and 1 bank for ITCM
                   "STR R1,[R0]\n"
    ...
    // Compiler gives a warning that we can't call main due to ISO C++
    // standards, but in our case this is not applicable as this in the reset
    // interrupt
...
WEAK void ENET_QOS_IRQHandler(void) { ENET_QOS_DriverIRQHandler(); }

WEAK void ENET_QOS_PMT_IRQHandler(void) { ENET_QOS_PMT_DriverIRQHandler(); }
